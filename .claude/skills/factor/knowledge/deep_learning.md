...
<!-- DAILY_COLUMNS -->
  ...
  - growth: growth因子描述（growth因子描述）
  - high: 最高价（日线最高价）
  - holder_change_pct: 股东户数变化率（股东户数较上期变化百分比，正值表示筹码分散）
  - jhjj_hsl: 集合竞价换手率（集合竞价时段换手率，单位：%）
  - low: 最低价（日线最低价）
  - market_cap: 总市值（单位：亿元，衡量公司规模）
  - momentum: momentum因子描述（momentum因子描述）
  - net_amount_main: 主力净流入金额（主力资金净流入金额）
  - net_margin: 净利率（Net Profit Margin, 衡量综合盈利效率）
  - net_pct_l: 大单净流入占比（大单（>=50万股或>=100万元）净流入占比）
  - net_pct_m: 中单净流入占比（中单净流入占比）
  - net_pct_main: 主力净流入占比（主力资金净流入占成交额比例）
  - net_pct_s: 小单净流入占比（小单净流入占比）
  - net_pct_xl: 超大单净流入占比（超大单（>=100万股或>=500万元）净流入占比）
  ...
  - quality: quality因子描述（quality因子描述）
