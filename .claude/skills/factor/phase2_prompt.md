...
**允许的推导只有一种**：用清单内的列做**精确的**数学运算。例如收益率 `close.pct_change()`、昨日收盘 `close.shift(1)`、复权价 `close * factor`、日内收益 `close / open - 1`。判据是：算出来的值与原文定义**逐点相等**，不是"大致相当"。

> 注意：**截面后处理**（排名/标准化/中性化）不属于缺字段，照常跳过、只输出个股原始值（这类处理最后统一做）。但"缺某个字段"永远不能跳过或近似。

- **需要的字段在清单里能找到 → 字段齐全**：
  - 挑出对应的真实列名（如"复权收盘价"对应 `close` × `factor`），这些就是本因子的 `{cols}`
  - 继续 Step 1 写代码，`--cols` 用这些真实列名
- **需要的字段不在清单里 → 看能否从清单内的列精确推导得到**：
  - 能精确推导 → 用推导式实现，正常写代码
  - 推不出来 → **判缺列**
- **判缺列后：停止**
