...
def _format_column_groups(groups: dict[str, dict[str, str]], available_columns: list[str]) -> str:
    ...
    # Also build a set without $ prefix for matching per-stock parquet files
    available_no_prefix = {c.lstrip("$") for c in available}
    lines = []
    for category, cols in groups.items():
        present = {}
        for k, v in cols.items():
            # Match with or without $ prefix
            if k in available or k.lstrip("$") in available_no_prefix:
                # Use the actual column name (without $) if available
                actual_name = k.lstrip("$") if k.lstrip("$") in available_no_prefix else k
                present[actual_name] = v
        if not present:
            continue
...
""")
    ...
    if p.name.endswith(".h5"):
        ...
        try:
            ...
            if isinstance(data, list):
                content = f"JSON 列表，共 {len(data)} 项。前 5 项: {data[:5]}"
            elif isinstance(data, dict):
                content = f"JSON 对象，键: {list(data.keys())[:10]}"
            else:
    ...
    if "minute" in p.name.lower() or "minute" in str(p.parent).lower():
    ...
    file_type = "HDF5 Data File" if p.name.endswith(".h5") else "Parquet Data File"
    return JJ_TPL.render(
        file_name=p.name,
        type_desc=file_type,
        content=df_info,
...
def get_data_folder_intro(fname_reg: str = ".*", flags=0, variable_mapping=None) -> str:
    ...
    pattern_by_mode = {
        "all": r"^(.+\.(parquet|h5)|README\.md|stock_list\.json|trade_dates\.json|jq_data_meta\.json)$" if fname_reg == ".*" else fname_reg,
        "daily": r"^(daily.*\.(parquet|h5)|README\.md|stock_list\.json|trade_dates\.json)$",
        "minute": r"^(minute.*\.(parquet|h5)|README\.md|stock_list\.json|trade_dates\.json)$",
    }
    content_l = []
    data_dir = Path(FACTOR_COSTEER_SETTINGS.data_folder_debug)
    # 遍历根目录和 stock_data 子目录
    search_dirs = [data_dir]
    stock_data_dir = data_dir / "stock_data"
    if stock_data_dir.exists():
        search_dirs.extend(stock_data_dir.iterdir())
...
def get_compact_data_folder_intro() -> str:
    ...
    def _stock_columns(freq: str) -> list[str]:
        """从 per-stock parquet 文件读取列名。"""
        stock_list_path = data_folder / "stock_data" / freq / "stock_list.json"
        ...
        try:
            ...
            if not stocks:
                return []
            sample_path = data_folder / "stock_data" / freq / f"{stocks[0]}.parquet"
            if sample_path.exists():
                return list(pd.read_parquet(sample_path).columns)
        except Exception:  # noqa: BLE001
            pass
        return []

    def _daily_intro() -> str:
        cols = _stock_columns("daily")
        if not cols:
            return "- 日线数据: 字段不可用"
        return "- 日线数据: per-stock parquet, DatetimeIndex\n" + _format_column_groups(
            DAILY_COLUMN_GROUPS, cols
        )

    def _minute_intro() -> str:
        cols = _stock_columns("minute")
        if not cols:
            return "- 分钟数据: 字段不可用"
        return "- 分钟数据: per-stock parquet, DatetimeIndex\n" + _format_column_groups(
